...
public class FunctionCodegen {
    ...
    private void generateBridge(
        ...
        // TODO: generate additional bridge
        boolean isVarargInvoke = JvmCodegenUtil.isDeclarationOrOverrideOfBigArityFunctionInvoke(descriptor);
        assert isVarargInvoke || argTypes.length == originalArgTypes.length :
                "Number of parameters of the bridge and delegate must be the same.\n" +
                "Descriptor: " + descriptor + "\nBridge: " + bridge + "\nDelegate: " + delegateTo;
        if (isVarargInvoke) {
            assert originalArgTypes.length == 1 && originalArgTypes[0].equals(AsmUtil.getArrayType(OBJECT_TYPE)) :
        ...
        for (int i = 0, reg = 1; i < originalArgTypes.length; i++) {
            StackValue value;
            if (isVarargInvoke) {
                value = StackValue.arrayElement(OBJECT_TYPE, null, StackValue.local(1, argTypes[i]), StackValue.constant(i));
            }
            else {
